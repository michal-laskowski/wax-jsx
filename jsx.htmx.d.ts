...
declare namespace HtmxUtils {
    type HxOnMap = { [K in keyof GlobalEventHandlersEventMap as `hx-on-${K}`]?: string } & {
        [K in HxOnHtmxEvents as `hx-on--${K}`]?: string
    }

    type HxOnHtmxEvents = JsxHtmxEvents | keyof { [K in keyof HxSubevents as `${K}-${HxSubevents[K]}`]: never }
    ...
    type KebabToCamel<T extends string> = T extends `${infer Head}-${infer Rest}`
